...
**24** of the flag definitions below live in `frontend/index.html` (calculator engine `FLAG_DEFS`): **F1–F17 and F20–F26**. A 25th, **F27**, is documented below as governance spec (added 2026-07-16 alongside `governance/PRICING_RULES.md` §32) but is **not yet implemented** in the engine — see its entry for scope and status. F18 and F19 (MOQ-era flags) were removed from the engine in Session B (2026-06-05) when MOQ was purged from the account; their rows were deleted from this table in Session H (they had survived here as ghost documentation — audit H-2/H-4). F26 was added in the 2026-07-15 governance drift remediation session (see below). Four defined flags are currently **unreachable by design** and documented as such below: F9 (retired under the §28 no-floor doctrine), F11 (retired with report-only enforcement), F12 (reserved — ink is always full bleed per §25), F16 (`computeLamPasses` can only return 1, 2, or STOP, never >2).
...
| F27 | REVIEW | no | **Added 2026-07-16 (nearest-neighbor chain-consistency rule, `governance/PRICING_RULES.md` §32).** New item's $/sq ft, at one or more of the six tiers, undercuts its nearest larger validated same-family neighbor's $/sq ft by 2 or more $0.25 increments (a Direction B violation beyond the §32 tolerance) — the class of deep-tier chain break the account's qty-20-only validation process does not otherwise catch (`audits/2026-07-16-deep-tier-chain-consistency.md`). Does not fire against the two permanent root benchmarks (P/N 1230820, 1205720) or against any item routed through the F26 per-label floor. Acknowledge in the validation prompt before proceeding — per §32, closing a flagged gap is a pricing decision, not an automatic recalculation. **Spec only as of this addition:** this entry defines the flag for governance purposes; the nearest-neighbor lookup is not yet wired into `buildPrintLamSinglesTiers`, the cut-vinyl tier builders, or `calculator_config.json` — implementing the live engine check is separate follow-up work. |
