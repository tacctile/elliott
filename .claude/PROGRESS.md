...
>
> Last Updated: 2026-06-09 (Session H — Pricing Engine v2: Supabase data layer migrated + seeded (7 elliott_* tables in the shared `prolabel` project, RLS applied and role-tested), Materials Manager tab added (full CRUD + combinations builder via anon key), calculator hydrates Supabase-first with static-JSON offline fallback + banner, audit findings C-1 (report-only enforcement — briefs band-anchored again), H-1–H-5, and W1–W6 all resolved, kit-family material costs normalized to §25 canonical ($3.60/$5.95, documentation-only), validate.py gains 4 checks and gates CI, sanity suite expanded to 14 cases — 14/14 pass. No sell prices, tiers, or band anchors changed.)
